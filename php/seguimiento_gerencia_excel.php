...
$objSpreadsheet->getProperties()->setTitle("seguimiento gerencia");
$objSpreadsheet->createSheet(0);
$objSpreadsheet->setActiveSheetIndex(0);
$objSpreadsheet->getActiveSheet()->setTitle("seguimiento gerencia");
...
$objSpreadsheet->getActiveSheet()->SetCellValue("E2", "REPORTE de seguimiento gerencia");
...
// colegio al dueño de HOY, no al de entonces). Cuando un mismo colegio+periodo tiene más de
// un cod_zona distinto entre sus líneas (dato ambiguo), se descarta esa resolución (HAVING
// COUNT(DISTINCT cod_zona) = 1) y se cae a colegios.cod_zona como respaldo, en vez de
// adivinar cuál aplica. Los admins (tipo=1) nunca cuentan como dueños de zona; Hector
// Morales (id=69, tipo=10) sí, por la misma excepción de negocio que ya aplican los demás
// endpoints del dashboard. Mismo criterio que php/valoriza_global_excel.php.
...
// Las 4 columnas propias de este reporte (última visita / muestras despachadas / propuesta
// comercial / acuerdo de adopción) van siempre después de las columnas de valorización
// global, corriéndose 2 letras cuando además se muestran las columnas de costo IA.
$col_visita    = $mostrar_ia_g ? 'R' : 'P';
$col_muestras  = $mostrar_ia_g ? 'S' : 'Q';
$col_propuesta = $mostrar_ia_g ? 'T' : 'R';
$col_acuerdo   = $mostrar_ia_g ? 'U' : 'S';
$col_fin       = $col_acuerdo;
...
$objSpreadsheet->getActiveSheet()->SetCellValue($col_visita."6", "Última visita");
$objSpreadsheet->getActiveSheet()->SetCellValue($col_muestras."6", "Muestras despachadas");
$objSpreadsheet->getActiveSheet()->SetCellValue($col_propuesta."6", "Propuesta comercial");
$objSpreadsheet->getActiveSheet()->SetCellValue($col_acuerdo."6", "Acuerdo de adopción cargado");


$objSpreadsheet->getActiveSheet()->getStyle("A6:{$col_fin}6")->applyFromArray([
    'fill' => [
...
// Venta real + acuerdo de adopción cargado viven en la misma fila de "recursos" (una por
// colegio+periodo), así que se resuelven juntos en una sola consulta.
$vreal_map_g   = [];
$acuerdo_map_g = [];
if (!empty($cole_ids_g)) {
    $req_vr_g = $bdd->prepare("SELECT id_colegio, venta_real, archivo, archivo2, archivo3 FROM recursos WHERE id_colegio IN ($ph_g) AND id_periodo=?");
    ...
    foreach ($req_vr_g->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $acuerdo_map_g[$row['id_colegio']] = (!empty($row['archivo']) || !empty($row['archivo2']) || !empty($row['archivo3']));
    }
}
...
// Última visita ejecutada al colegio (sin restringir al período del reporte: es un indicador
// de recencia general para seguimiento gerencial, no un valor propio del período — mismo
// criterio ya usado para "último contacto" en php/cubrimiento_excel2.php).
$visita_map_g = [];
if (!empty($cole_ids_g)) {
    $req_visita_g = $bdd->prepare("SELECT p.id_colegio, MAX(v.fecha) as ultima_visita FROM plan_trabajo p JOIN visitas v ON p.id=v.id_plan_trabajo WHERE p.id_colegio IN ($ph_g) AND p.resultado=1 GROUP BY p.id_colegio");
    $req_visita_g->execute($cole_ids_g);
    foreach ($req_visita_g->fetchAll(PDO::FETCH_ASSOC) as $row)
        $visita_map_g[$row['id_colegio']] = $row['ultima_visita'];
}

// Muestras despachadas: tabla "muestreos" (solicitudes de muestra) en estado=4, que es
// literalmente el estado "Muestreo despachado correctamente" (ver php/accion_muestreo.php).
// Se cuenta la cantidad aprobada de cada línea de libro (o la solicitada si nunca se aprobó
// una cantidad distinta), igual que hace el reporte de muestreo (php/muestreo_f_excel.php).
$muestras_map_g = [];
if (!empty($cole_ids_g)) {
    $req_muestras_g = $bdd->prepare("SELECT m.id_colegio, SUM(IF(lm.cantidad_aprob=0, lm.cantidad, lm.cantidad_aprob)) as total
        FROM muestreos m JOIN libros_muestreos lm ON lm.cod_muestreo=m.codigo
        WHERE m.id_colegio IN ($ph_g) AND m.id_periodo=? AND m.estado=4
        GROUP BY m.id_colegio");
    $req_muestras_g->execute(array_merge($cole_ids_g, [$_POST['periodo']]));
    foreach ($req_muestras_g->fetchAll(PDO::FETCH_ASSOC) as $row)
        $muestras_map_g[$row['id_colegio']] = (int)$row['total'];
}

// Propuesta comercial: existencia de un adjunto tipo=1 para el colegio+periodo (mismo
// criterio que colegio.php y php/cubrimiento_excel2.php).
$propuesta_map_g = [];
if (!empty($cole_ids_g)) {
    $req_prop_g = $bdd->prepare("SELECT id_colegio FROM adjuntos WHERE id_colegio IN ($ph_g) AND id_periodo=? AND tipo=1 GROUP BY id_colegio");
    $req_prop_g->execute(array_merge($cole_ids_g, [$_POST['periodo']]));
    foreach ($req_prop_g->fetchAll(PDO::FETCH_ASSOC) as $row)
        $propuesta_map_g[$row['id_colegio']] = true;
}
...
foreach ($colegios as $colegio) {
    ...
    $ultima_visita_val = $visita_map_g[$colegio["id"]] ?? '';
    $objSpreadsheet->getActiveSheet()->SetCellValue($col_visita.$conta, $ultima_visita_val);
    $objSpreadsheet->getActiveSheet()->SetCellValue($col_muestras.$conta, $muestras_map_g[$colegio["id"]] ?? 0);
    $objSpreadsheet->getActiveSheet()->SetCellValue($col_propuesta.$conta, !empty($propuesta_map_g[$colegio["id"]]) ? 'Sí' : 'No');
    $objSpreadsheet->getActiveSheet()->SetCellValue($col_acuerdo.$conta, !empty($acuerdo_map_g[$colegio["id"]]) ? 'Sí' : 'No');
...
header('Content-Disposition: attachment; filename="Seguimiento_gerencia.xlsx"');
