...
$titulo      = '';
$mensaje     = '';
$redirect    = '../lista_pedidos.php?tp=2';
...
try {
    if (isset($_GET['rechazar'])) {
        ...
        $bdd->prepare("UPDATE pedidos SET estado = '3' WHERE id = ?")->execute([$id]);
        registrar_historial_estado($bdd, 'pedidos', $id, 3, $id_usuario_accion);
        $titulo   = '¡Pedido rechazado!';
        $mensaje  = "El pedido #$id fue rechazado/anulado correctamente.";
        $redirect = '../lista_pedidos.php?tp=2';

    } elseif (isset($_GET['aprobar'])) {
        $id = intval($_GET['aprobar']);
        $bdd->prepare("UPDATE pedidos SET estado = '2', observaciones = ? WHERE id = ?")
            ->execute([$_GET['observaciones'] ?? '', $id]);
        registrar_historial_estado($bdd, 'pedidos', $id, 2, $id_usuario_accion);
        $titulo   = '¡Pedido aprobado!';
        $mensaje  = "El pedido #$id fue aprobado correctamente.";
        ...
        registrar_historial_estado($bdd, 'pedidos', $id, 6, $id_usuario_accion);
        ...
        $bdd->prepare("UPDATE pedidos SET estado = '7' WHERE id = ?")->execute([$id]);
        registrar_historial_estado($bdd, 'pedidos', $id, 7, $id_usuario_accion);
...
<head>
  <meta charset="utf-8">
  <title>Inkpulse - Acción pedido</title>
