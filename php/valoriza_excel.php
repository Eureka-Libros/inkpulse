...
foreach ($colegios as$colegio) {
    ...
    if ($colegio["id_grado"] != 17 && $colegio["cod_area"]=="") {
        ...
        if (!isset($cache_alumnos[$key])) {
            $sq_gp = "SELECT SUM(alumnos) as alumnos FROM grados_paralelos 
              WHERE id_colegio = ? AND id_grado = ? AND id_periodo = ? AND alumnos > 0";

            $req_gp = $bdd->prepare($sq_gp);
            $req_gp->execute([$colegio["id"], $colegio["id_grado"], $_POST['periodo']]);
        ...
        if (!isset($cache_alumnos[$key])) {
            ...
            $req_gp = $bdd->prepare($sq_gp);
            $req_gp->execute([$colegio["id"], $grado_o["id_grado_otro"], $_POST['periodo']]);
            $gp = $req_gp->fetch(PDO::FETCH_ASSOC);
            $cache_alumnos[$key] = isset($gp['alumnos']) && $gp['alumnos'] !== null ? $gp['alumnos'] : 0;
        }
